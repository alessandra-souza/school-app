...
function displayArray(data,pNum=0) {
 ...
 function postDataStudent(formElement)
{
    ...
    formElement.submit(function(evt)
    {
        ...
        ({
            ...
            {
                // populate data
                $('#btn-add-student').click(getDataStudents());
                // reset form
                thisForm[0].reset();
                // display output
                if (response.error)
...
$(document).ready(function () {
 ...
 var formElement = $('#student-form');

    if (formElement.length)
    {
        // Triger  form submission handler
        postDataStudent(formElement);
    }   

    setTimeout(getDataStudents, 500);
